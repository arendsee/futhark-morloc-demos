...
A small neural network (784 -> 32 -> 10) trained by gradient descent on the real
MNIST digits. Every matmul
...
- **Python** (`mnist.py`): download the real MNIST digits (cached under `data/`),
  serve a deterministic subset (784-dim, scaled to [0,1]), and plot the confusion
  matrix (data access + viz).
- **morloc** (`main.loc`): the epoch loop, threading the two weight matrices
  (`Matrix 784 32 F32`, `Matrix 32 10 F32`) and dropping the per-step loss.
